 ...
 namespace impl {
  ...
  /*
   ...
   struct wrap_infty  {// : tag::is_gf<DomainType>  {
    static_assert(DomainType::has_tail,"Oops...");
    high_frequency_expansion_view tail;
    wrap_infty(high_frequency_expansion_view const & t): tail(t) {}
    operator domains::infty() const { return domains::infty();}
    typename high_frequency_expansion_view::const_result_type operator() ( typename high_frequency_expansion_view::arg0_type const & x) const { return tail(x);}
...
*/
  ...
  template<typename DomainType, bool IsView> class gf_impl { //: tag::is_gf<DomainType> {
   ...
   protected:
    ...
    template<typename GfType> gf_impl(GfType const & x){*this =x;}
    ...
    template<typename RHS> void operator = (RHS const & rhs) { 
     ...
     dom = rhs.domain(); data = rhs(arg_range_type()) ; tail= rhs(domains::infty()); 
    } 
   ...
   public:
    ...
    typedef typename domain_type::index_type arg_range_type;
   ...
   public:
    ...
    friend void h5_read( H5::CommonFG file, std::string path, gf & g) {} // exceptions will propagate from the array read/write... 
  ...
  template<typename meshes::tail> struct cast_to_infty  { operator domains::infty() const { return domains::infty();} };

  }// namespace impl 

  // --------------------------- THE USER CLASSES ------------------------------------------------------
  ///The regular class of GF
  template<typename DomainType> class gf : public impl::gf_impl<DomainType,false>, public cast_to_infty<DomainType> {
  ...
  template<typename DomainType> class gf_view : public impl::gf_impl<DomainType,true>, public cast_to_infty<DomainType> {
  ...
  template <typename M> struct is_gf< gf<M> >:mpl::true {};
  template <typename M> struct is_gf< gf_view<M> >:mpl::true {};
  ...
  struct get_mesh {
   BOOST_PROTO_CALLABLE();
   template<typename Sig> struct result;
   template<typename This, typename X> struct result<This(X)> { typedef typename X::domain_type type;};
  ...
  template<typename TAG>
   struct combine_mesh {
    BOOST_PROTO_CALLABLE();
    template<typename Sig> struct result;
    template<typename This, typename M1, typename M2> 
     struct result<This(M1,M2)> { typedef mpl::if_<boost::is_same<M1,no_mesh>, M2, M1>::type type;};
    template<typename D> typename operator ()(no_mesh const & m1, D const & m2) const { return m2;}
    template<typename D> typename operator ()(D const & m1, no_mesh const & m2) const { return m1;}
    // here write out policy
    template<typename M1, typename M2> typename operator ()(M1 const & m1, M2 const & m2) const { 
    ...
    struct dom_t : 
     ...
     // ,proto::when< ElementGrammar, combine_mesh(get_mesh(proto::_value), proto::_state)  >
     proto::when< ScalarGrammar, no_mesh() >
     ,proto::when< ElementGrammar, get_mesh(proto::_value) >
     ,proto::when< proto::plus <dom_t,dom_t>,       combine_mesh<tag::plus>       (dom_t(proto::_left), dom_t( proto::_right)) >
     ,proto::when< proto::minus <dom_t,dom_t>,      combine_mesh<tag::minus>      (dom_t(proto::_left), dom_t( proto::_right)) >
     ,proto::when< proto::multiplies <dom_t,dom_t>, combine_mesh<tag::multiplies> (dom_t(proto::_left), dom_t( proto::_right)) >
     ,proto::when< proto::divides <dom_t,dom_t>,    combine_mesh<tag::divides>    (dom_t(proto::_left), dom_t( proto::_right)) >
    ...
    /*
       ...
       BOOST_PROTO_CALLABLE();
       template<typename Sig> struct result;
       template<typename This, typename A, typename D> struct result<This(A,D)> : boost::result_of<combine_mesh(typename get_mesh_type<A>::type,D)> {};
       template<typename A, typename D> typename  operator ()(A const &a, D const & d) const { return combine_mesh(a.domain(),d); }
       };

       template<typename L, typename R>  static DomainType compute_mesh (L const & dl, R const & dr) 
    //{ if (!(dl==dr)) TRIQS_RUNTIME_ERROR <<"domain mismatch"; return dl; }
    {  return dl; }
    template<typename L> static DomainType compute_mesh (L const & dl, no_mesh) { return dl; }
    template<typename R> static DomainType compute_mesh (no_mesh, R const & dr) { return dr; }
    */

    // grammar and domain and proto expression
    template <typename Expr> struct gf_expr;
    typedef tup::domain<gf_grammar,gf_expr,true> gf_mesh;

    typedef eval_transform<is_gf, is_scalar_or_element,eval_fnt<1> > eval1;
    template<typename Expr> struct gf_expr : boost::proto::extends<Expr, gf_expr<Expr>, gf_mesh>{
     gf_expr( Expr const & expr = Expr() ) : boost::proto::extends<Expr, gf_expr<Expr>, gf_mesh>  ( expr ) {}
     template<typename T> 
      typename boost::result_of<eval1 >(Expr,bf::vector<T>) >::type operator() (T const & x) const {return eval1()(*this, bf::make_vector(x));}
     template<typename T> 
      typename boost::result_of<dom_t >(Expr,no_mesh) >::type operator() (T const & x) const { return dom_t()(*this, no_mesh()); }
  ...
  template <typename Expr> struct gf_expr< Expr >:mpl::true {};

  // BOOST macro for all domains...
#define AUX(r,data,DOM)  BOOST_PROTO_DEFINE_OPERATORS(expr_templ<meshes::DOM>::is_gf, expr_templ<meshes::DOM>::gf_mesh);
...
/*
  template< typename DomainType> struct node_desc_impl {// algebra node description  
   ...
   template<typename S> struct scalar {
    ...
    typedef no_mesh domain_type;
    no_mesh domain () const {} 
   };
   template<typename L, typename R> struct plus { 
    ...
    typedef DomainType domain_type;
    DomainType domain () const { return compute_mesh(l.domain(),r.domain());}
   }; 
