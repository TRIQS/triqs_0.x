...
namespace triqs { namespace gf { 
 ...
 template<typename Descriptor,bool IsView> class gf_impl : Descriptor::tag {
  public : 
   ...
   typedef typename Descriptor::auxiliary_data_t  auxiliary_data_t;
   typedef typename Descriptor::target_t          target_t;
   typedef typename Descriptor::target_view_t     target_view_t;
   ...
   typedef typename auxiliary_data_t::non_view_type                                    aux_data_non_view_t;
   typedef typename auxiliary_data_t::view_type                                        aux_data_view_t;
   typedef typename mpl::if_c<IsView, aux_data_view_t, aux_data_non_view_t>::type   aux_data_t;
   ...
   aux_data_view_t aux_data_view()             { return aux_data;} 
   const aux_data_view_t aux_data_view() const { return aux_data;}
  ...
  protected:
   ...
   template<typename... AuxArgs>
    gf_impl(mesh_t const & m, shape_t const & sh, AuxArgs... aux_args): _mesh(m), data(sh.append(m.size())), aux_data(sh,aux_args... ){data()=0;}
   gf_impl(mesh_t const & m, shape_t const & sh, aux_data_view_t const & t): _mesh(m), data(sh.append(m.size())), aux_data(t){data()=0;}
   gf_impl(mesh_t const & m, data_view_t const & dat, aux_data_view_t const & ad) : _mesh(m), data(dat), aux_data(ad){}

   gf_impl() {} // all arrays of zero size (empty)
   gf_impl(gf_impl const & x)                    : _mesh(x._mesh), data(x.data), aux_data(x.aux_data){}
   gf_impl(gf_impl<Descriptor,!IsView> const & x): _mesh(x._mesh), data(x.data), aux_data(x.aux_data){} 

  public:

   void operator = (gf_impl const & rhs)                     { _mesh = rhs._mesh; data = rhs.data; aux_data = rhs.aux_data;}
   void operator = (gf_impl<Descriptor,!IsView> const & rhs) { _mesh = rhs._mesh; data = rhs.data; aux_data = rhs.aux_data;}
   ...
   template<typename RHS> void operator = (RHS const & rhs) { // the general version  
    ...
    Descriptor::assign_from_expression(_mesh,data,aux_data,rhs);
   }
   ...
   template<typename Arg0, typename... Args >    // match any argument list, picking out the first type : () is not permitted
    ...
    clef::one_is_lazy<Arg0, Args...>                          // One of Args is a lazy expression
     >,                                                       // end of OR 
    std::result_of<Descriptor(mesh_t, data_t, aux_data_t, Arg0, Args...)> // what is the result type of call
     >::type     // end of lazy_disable_if 
     operator() (Arg0&& arg0, Args&&... args) const {return Descriptor()(_mesh, data, aux_data, std::forward<Arg0>( arg0), std::forward<Args>(args)...);}
   ...
   target_view_t on_grid (Args... args)       { return (*this)( _mesh[mesh_index_t(args...)]);}

   /// A direct access to the grid point (const version)
   template<typename... Args> 
    target_view_t on_grid (Args... args) const { return (*this)( _mesh[mesh_index_t(args...)]);}

   // Interaction with the CLEF library : calling the gf with any clef expression as argument build a new clef expression
   template<typename ...Args>
    typename std::result_of<clef::lazy_call<view_type>(Args...)>::type
    operator()(Args... args) const { 
     static_assert(sizeof...(Args) == Descriptor::arity, "Incorrect number of variable in call");
   ...
   template<typename F> friend void triqs_nvl_auto_assign (gf_impl & x, F f) { Descriptor::assign_from_expression(x._mesh,x.data, x.aux_data,f);}
   ...
   friend void h5_write (tqa::h5::group_or_file fg, std::string subgroup_name, gf_impl const & g) {
    ...
    h5_write(gr,Descriptor::auxiliary_data_name(),g.aux_data);
   ...
   friend void h5_read  (tqa::h5::group_or_file fg, std::string subgroup_name, gf_impl & g){
    ...
    h5_read(gr,Descriptor::auxiliary_data_name(),g.aux_data);
   ...
   template<class Archive>
    void serialize(Archive & ar, const unsigned int version) {
     ...
     ar & boost::serialization::make_nvp(Descriptor::auxiliary_data_name(),aux_data);
 ...
 template<typename Descriptor> class gf : public gf_impl<Descriptor,false> {
  ...
  template<typename... AuxArgs> gf( mesh_t const & m,typename B::shape_t sh, AuxArgs&&... args) : B(m,sh,std::forward<AuxArgs>(args)...) {}
  gf( mesh_t const & m,typename B::shape_t sh, typename Descriptor::auxiliary_data_t const & t) : B(m,sh,t) {}
 ...
 template<typename Descriptor> class gf_view : public gf_impl<Descriptor,true> {
  ...
  template<typename D, typename T> gf_view (mesh_t const & m, D const & dat,T const & t) : B(m,dat,t) {}
  using B::operator=;// or the default is = synthetized...
 };

 template<typename Descriptor, bool V, typename... Args> 
  gf_view<Descriptor> slice_target (gf_impl<Descriptor,V> const & g, Args... args) {
   return gf_view<Descriptor>(g.mesh(), g.data_view()(args... , tqa::range()), slice_target (g.aux_data_view(),args...)); 
  }

 template<typename Descriptor, bool V, typename... Args> 
  gf_view<Descriptor> slice_mesh (gf_impl<Descriptor,V> const & g, Args... args) {
   return gf_view<Descriptor>(g.mesh().slice(args...), g.data_view()(arrays::ellipsis(), g.mesh().slice_get_range(args...)), g.aux_data_view()); 
  }
