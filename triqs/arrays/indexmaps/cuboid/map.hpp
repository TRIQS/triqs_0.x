 ...
 template<int Rank> ull_t memory_layout_from_strides(mini_vector<std::ptrdiff_t, Rank> const & strides) {
  ...
  for (size_t i=0; i<Rank; ++i)
   for (size_t j=i+1; j<Rank; ++j) 
    if (strides[i] > strides[j]) c[i]++;
  return permutations::permutation_from_array(c, Rank);
 } 
 ...
 */
 template<int Rank, ull_t OptionsFlags, ull_t TraversalOrder >
  class map {
   ...
   //map (memory_layout<Rank> ml = memory_layout<Rank>()):mydomain(), start_shift_(0) {} 
   //map ():mydomain(), start_shift_(0), memory_order_('C')  {} 
  ...
  map(domain_type const & C):
   // mydomain(C), start_shift_(0) { compute_stride_compact( mem_layout::c_order(Rank) ); } 
  ...
  map(domain_type const & C, memory_layout<Rank> ml):
   // mydomain(C), start_shift_(0) { compute_stride_compact(ml.value); } 
  ...
  void compute_stride_compact() {
   //last_index_ = mem_layout::memory_rank_to_index(memory_layout, rank-1);    
   size_t str = 1;
