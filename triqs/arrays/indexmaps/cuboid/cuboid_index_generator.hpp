...
namespace triqs { namespace arrays { namespace indexmaps { 
 ...
 class cuboid_index_generator {
   public:
    ...
    cuboid_index_generator (): Parent(NULL), atend(true) {}
    cuboid_index_generator (const domain_type & P, bool atEnd=false): Parent(&P), atend(atEnd) {}
    cuboid_index_generator & operator++(){ 
     cuboid_domain_generator_details::index_advance<IterationOrder,domain_type::rank,domain_type::rank>::do_it(indices_tuple,Parent,atend);
     return *this;
    }
    bool operator==(const cuboid_index_generator & IT2) const { return ((IT2.Parent == Parent) && (IT2.atend==atend) );}
 ...
 namespace cuboid_domain_generator_details {
  template<typename Perm, int rank, int v> struct index_advance {
   ...
   template<class I, class D> static void do_it(I & index, D const * Parent, bool & atend) {
#ifdef TRIQS_ARRAYS_ENFORCE_BOUNDCHECK
    if (atend) TRIQS_RUNTIME_ERROR << "Iterator in cuboid can not be pushed after end !";
#endif
    if (index[p] < Parent->lengths()[p]-1) { ++(index[p]); return; }
    index[p] = 0; index_advance<Perm, rank,v-1>::do_it(index, Parent,atend);
   }
  };
