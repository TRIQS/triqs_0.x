...
namespace triqs { namespace arrays {
 ...
 template <typename ValueType, ull_t Opt=0> class vector_qview;
...
#define IMPL_TYPE indexmap_storage_pair< indexmaps::cuboid::map<1,Opt,0> , storages::shared_block<ValueType>, Opt, 0, Tag::vector_view >
 ...
 template <typename ValueType, ull_t Opt >
  ...
  public :
   ...
   /// Swap
   friend void swap( vector_view & A, vector_view & B) noexcept { A.swap_me(B);}

   /// Rebind the view
   void rebind (vector_view const & X) noexcept { this->indexmap_ = X.indexmap_; this->storage_ = X.storage_;}
...
#define IMPL_TYPE indexmap_storage_pair< indexmaps::cuboid::map<1,Opt,0> , storages::shared_block_ref<ValueType>, Opt, 0, Tag::vector_view >

 /** */
 template <typename ValueType, ull_t Opt >
  class vector_qview : Tag::vector_view, TRIQS_MODEL_CONCEPT(MutableVector), public  IMPL_TYPE {
  public :
   typedef arrays::vector_view<ValueType,Opt> view_type;
   typedef vector<ValueType,Opt> non_view_type;
   typedef void has_view_type_tag;
   ...
   /// Build from an IndexMap and a storage
   template<typename S, ull_t Opt2, ull_t To2> vector_qview (indexmaps::cuboid::map<1, Opt2,To2> const & Ind,S const & Mem): IMPL_TYPE(Ind, Mem) {}

   /// Build from anything that has an indexmap and a storage compatible with this class
   template<typename ISP> vector_qview(const ISP & X): IMPL_TYPE(X.indexmap(),X.storage()) {}

   vector_qview(vector_qview const & X): IMPL_TYPE(X.indexmap(),X.storage()) {}
   vector_qview () = delete;
   vector_qview(vector_qview && X) noexcept { this->swap_me(X);}
   friend void swap( vector_qview & A, vector_qview & B) noexcept { A.swap_me(B);}

   /** Assignment.  The size of the array MUST match exactly.  */
   template<typename RHS> vector_qview & operator=(const RHS & X) { triqs_arrays_assign_delegation(*this,X); return *this; }

   vector_qview & operator=(vector_qview const & X) {triqs_arrays_assign_delegation(*this,X); return *this; }//cf array_view class comment
   // Move assignment not defined : will use the copy = since view must copy data

   size_t size() const { return this->shape()[0];}
   std::ptrdiff_t stride() const { return this->indexmap().strides()[0];}

   TRIQS_DEFINE_COMPOUND_OPERATORS(vector_qview);
...
#undef IMPL_TYPE
 ...
 template < class V, int R,  ull_t OptionFlags, ull_t To > struct ViewFactory< V, R,OptionFlags,To, Tag::vector_view> { typedef vector_qview<V,OptionFlags> type; };
 // ---------------------- vector--------------------------------
 ...
 template <typename ValueType, ull_t Opt>
  class vector: Tag::vector,  TRIQS_MODEL_CONCEPT(MutableVector), public IMPL_TYPE {
   public :
    ...
    /// Move assignment
    vector & operator=(vector && X) noexcept { this->swap_me(X); return *this;}
...
namespace triqs { namespace arrays {
 ...
 template <typename V, ull_t S1, ull_t S2>
  void deep_swap(vector_view <V,S1> x, vector_view<V,S2> y) { blas::swap(x,y); }

 template <typename V, ull_t S1, ull_t S2>
  void deep_swap(vector_qview <V,S1> x, vector_qview<V,S2> y) { blas::swap(x,y); }
...
namespace std {
 ...
 template <typename V, triqs::ull_t S> void swap( triqs::arrays::vector_qview<V,S> & a , triqs::arrays::vector_qview<V,S> & b)= delete;
}
#endif
