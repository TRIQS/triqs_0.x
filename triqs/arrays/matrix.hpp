...
namespace triqs { namespace arrays {
 ...
 template <typename ValueType, ull_t Opt=0, ull_t TraversalOrder= 0> class matrix_qview;
 ...
 template <typename ValueType, ull_t Opt, ull_t TraversalOrder >
  class matrix_view : Tag::matrix_view,  TRIQS_MODEL_CONCEPT(MutableMatrix), public IMPL_TYPE {
   public :
    ...
    /// Swap
    friend void swap( matrix_view & A, matrix_view & B) noexcept { A.swap_me(B);}

    /// Rebind the view
    void rebind (matrix_view const & X) noexcept { this->indexmap_ = X.indexmap_; this->storage_ = X.storage_;}
...
#define IMPL_TYPE indexmap_storage_pair < indexmaps::cuboid::map<2,Opt,TraversalOrder>, \
 storages::shared_block_ref<ValueType>, Opt, TraversalOrder, Tag::matrix_view >

 template <typename ValueType, ull_t Opt, ull_t TraversalOrder >
  class matrix_qview : Tag::matrix_view,  TRIQS_MODEL_CONCEPT(MutableMatrix), public IMPL_TYPE {
   public :
    typedef arrays::matrix_view<ValueType,Opt,TraversalOrder> view_type;
    typedef matrix<ValueType,Opt,TraversalOrder>      non_view_type;
    typedef void has_view_type_tag;
    ...
    /// Build from an IndexMap and a storage
    template<typename S> matrix_qview (typename IMPL_TYPE::indexmap_type const & Ind,S const & Mem): IMPL_TYPE(Ind, Mem) {}

    /// Build from anything that has an indexmap and a storage compatible with this class
    template<typename ISP> matrix_qview(const ISP & X): IMPL_TYPE(X.indexmap(),X.storage()) {}

    matrix_qview( matrix_qview const & X): IMPL_TYPE(X.indexmap(),X.storage()) {}
    matrix_qview () = delete;
    matrix_qview(matrix_qview && X)  noexcept { this->swap_me(X); }
    friend void swap( matrix_qview & A, matrix_qview & B) noexcept { A.swap_me(B);}

    /** Assignement.  The size of the array MUST match exactly.  */
    template<typename RHS> matrix_qview & operator=(const RHS & X) {triqs_arrays_assign_delegation(*this,X); return *this; }

    matrix_qview & operator=(matrix_qview const & X) {triqs_arrays_assign_delegation(*this,X); return *this; }//cf array_view class comment
    // Move assignment not defined : will use the copy = since view must copy data

    TRIQS_DEFINE_COMPOUND_OPERATORS(matrix_qview);
    _IMPL_MATRIX_COMMON;
  };
 ...
 storages::shared_block<ValueType>, Opt, TraversalOrder, Tag::matrix_view >
  ...
  class matrix: Tag::matrix,  TRIQS_MODEL_CONCEPT(MutableMatrix), public IMPL_TYPE {
   public :
    ...
    /// Move assignment
    matrix & operator=(matrix && X) noexcept { this->swap_me(X); return *this;}

    /// Swap
    friend void swap( matrix & A, matrix & B) noexcept { A.swap_me(B);}
...
#undef IMPL_TYPE
 ...
 template < class V, int R, ull_t OptionFlags, ull_t To > struct ViewFactory< V, R, OptionFlags, To, Tag::matrix_view> {
  typedef typename std::conditional <R == 1, vector_qview<V,OptionFlags >, matrix_qview<V,OptionFlags, To > >::type type;
 };
...
namespace std {
 ...
 template <typename V, triqs::ull_t Opt, triqs::ull_t To >
  void swap( triqs::arrays::matrix_qview<V,Opt,To> & a , triqs::arrays::matrix_qview<V,Opt,To> & b)= delete;
}
